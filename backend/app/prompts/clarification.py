...
3. 원본 질의에 사용자 불확실성 표현이 있으면 rag_ready는 반드시 false
   ...
   ※ "아무거나", "상관없어요", "그냥 추천해줘", "아무거나 좋아" 는 이 목록에 해당하지 않음.
     이 표현은 '어떤 것이든 괜찮다'는 수용 의사이지 방향을 모르는 것이 아님 → 절대 규칙 3 미적용
...
**특별 규칙 — topic_subject 재질문 금지 (예외 없음):**
asked_slots에 "topic_subject"가 있으면, topic이 여전히 대분류 수준이어도 반드시 rag_ready=true로 결정하세요.
사용자가 세부 장르를 특정하지 않겠다는 의사를 표현한 것이므로 현재 topic 범위로 검색을 진행합니다.
→ 이 경우 반드시 slots_to_ask=[], rag_ready=true
→ "아무거나" 응답이어도, topic이 대분류여도, 다른 조건이 뭐든 이 규칙은 최우선 적용됨
