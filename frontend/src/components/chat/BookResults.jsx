 ...
 */
export default function BookResults({ results = [], alsoResults = [], availabilityIndex = {} }) {
  const navigate = useNavigate();
  const [expandedIsbn, setExpandedIsbn] = useState(null);

  if (results.length === 0 && alsoResults.length === 0) return null;
      ...
      {results.length > 0 && (
        <>
          ...
          <div className="flex flex-col gap-3">
            {results.map((book, i) => renderCard(book, i))}
          </div>
        </>
      )}

      {alsoResults.length > 0 && (
        <div className="mt-5">
          <p className="text-[10px] tracking-widest uppercase text-ink-muted mb-1">이런 책도 있어요</p>
          <p className="text-[11px] text-ink-muted/70 mb-3">현재 대출 중이지만 적합도가 높은 책들이에요</p>
          <div className="flex flex-col gap-3">
            {alsoResults.map((book, i) => renderCard(book, i))}
          </div>
        </div>
      )}
